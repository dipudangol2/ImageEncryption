...
def test_manual_compressor():
    ...
    for size in test_sizes:
        ...
        for quality in [25, 50, 75, 90]:
            ...
            print(
                f"  Q{quality:2d}: {stats['original_size_display']} → {stats['compressed_size_display']} "
            ...
            if stats["compression_ratio"] > 2.0:
                print(
                    f"       ✅ Good compression: {stats['space_saved_percent']:.1f}% saved"
                )
            elif stats["compression_ratio"] > 1.2:
                print(
                    f"       ⚠️  Moderate compression: {stats['space_saved_percent']:.1f}% saved"
                )
            else:
                print(
                    f"       ❌ Poor compression: {stats['space_saved_percent']:.1f}% saved"
                )
