...
class ManualDCTCompressor:
        ...
        for u in range(N):
            for x in range(N):
                if u == 0:
                    ...
                    dct_matrix[u, x] = np.sqrt(2.0 / N) * np.cos((2*x + 1) * u * np.pi / (2*N))
    ...
    def _get_quantization_matrix(self, quality: int, is_luminance: bool = True) -> np.ndarray:
    ...
    def _compress_channel(self, channel: np.ndarray, quality: int, is_luminance: bool = True) -> Tuple[np.ndarray, bytes]:
        ...
        for i in range(0, h, 8):
            for j in range(0, w, 8):
                ...
                compressed_blocks.append({
                    'indices': non_zero_indices.astype(np.uint8),  # Position in 64-element block
        ...
        reconstructed = self._reconstruct_from_compressed_blocks(compressed_blocks, (h, w), quant_matrix)
    ...
    def _reconstruct_from_compressed_blocks(self, compressed_blocks: list, padded_shape: Tuple[int, int], quant_matrix: np.ndarray) -> np.ndarray:
    ...
    def compress(self, image: np.ndarray, quality: int = 75, use_color: bool = True) -> Dict:
        ...
        Args:
            ...
            use_color: Whether to process as color (compatibility parameter)
        
        Returns:
            Compressed data dictionary with accurate size reporting
        """
        ...
        if len(image.shape) == 2:
            ...
            compressed_data = {
                ...
                'quantization_matrix': self._get_quantization_matrix(quality, True).astype(np.float16),
            ...
            for ch in range(c):
                channel = image[:, :, ch]
                # Use luminance quantization for all channels (could differentiate chrominance)
                reconstructed_ch, coeffs_bytes = self._compress_channel(channel, quality, True)
    ...
    def decompress(self, compressed_data: Dict) -> np.ndarray:
        """
        Decompress image data by reconstructing from quantized coefficients.
        
        This provides real decompression instead of just storing the result.
        """
        ...
        if num_channels == 1:
            ...
            for ch in range(c):
                ...
                reconstructed_ch = self._decompress_channel(channel_coeffs, (h, w), quality, True)
    ...
    def _decompress_channel(self, coeffs_bytes: bytes, original_shape: Tuple[int, int], quality: int, is_luminance: bool = True) -> np.ndarray:
        ...
        for _ in range(num_blocks):
            ...
            # Read indices
            indices = np.frombuffer(coeffs_bytes[offset:offset+num_coeffs], dtype=np.uint8)
            offset += num_coeffs
            
            # Read values
            values = np.frombuffer(coeffs_bytes[offset:offset+num_coeffs*2], dtype=np.int16)
            offset += num_coeffs * 2
        ...
        # Reconstruct image
        reconstructed = self._reconstruct_from_compressed_blocks(compressed_blocks, (padded_h, padded_w), quant_matrix)
    ...
    def get_compression_stats(self, original_image: np.ndarray, compressed_data: Dict) -> Dict:
        """
        Calculate compression statistics with FIXED unit handling.
        
        This fixes the 340KB → 34000KB bug by ensuring proper units.
        """
        ...
        # Calculate ratios
        compression_ratio = original_size_bytes / total_compressed_size if total_compressed_size > 0 else 1.0
        space_saved_percent = (1 - total_compressed_size / original_size_bytes) * 100 if original_size_bytes > 0 else 0
...
def test_manual_compressor():
    """Test the manual DCT compressor with quantization."""
    print("🧪 Testing Manual DCT Compressor with Quantization")
    print("=" * 60)
    ...
    for size in test_sizes:
        print(f"\n📸 Testing {size[0]}x{size[1]} color image:")
        ...
        for quality in [25, 50, 75, 90]:
            ...
            print(f"  Q{quality:2d}: {stats['original_size_display']} → {stats['compressed_size_display']} "
                  f"({stats['compression_ratio']:.2f}x) PSNR: {psnr:.1f} dB")
            
            # Check compression effectiveness
            if stats['compression_ratio'] > 2.0:
                print(f"       ✅ Good compression: {stats['space_saved_percent']:.1f}% saved")
            elif stats['compression_ratio'] > 1.2:
                print(f"       ⚠️  Moderate compression: {stats['space_saved_percent']:.1f}% saved")
            else:
                print(f"       ❌ Poor compression: {stats['space_saved_percent']:.1f}% saved")
    
    print(f"\n✅ Manual DCT compressor test completed!")


if __name__ == "__main__":
    test_manual_compressor()